...
import { Box, TextField, Button, InputAdornment, Tooltip, Chip, CircularProgress, MenuItem } from '@mui/material';
...
type CatOption = { id: number | 'none'; label: string; value: number | null };
...
const InversionToolbar: React.FC<Props> = ({
  ...
  const [catOptions, setCatOptions] = useState<CatOption[]>([{ id: 'none', label: 'Todas', value: null }]);
  ...
  const fetchCategories = async (q: string) => {
    try {
      ...
      const opts: CatOption[] = [{ id: 'none', label: 'Todas', value: null }, ...list.map(c => ({ id: c.id, label: c.nombre, value: c.id }))];
      setCatOptions(opts);
    ...
    <Box mb={6}>
      ...
      <Box display="flex" flexWrap="wrap" gap={2} mb={2} alignItems="center">
        ...
        <TextField
          ...
          InputProps={{ endAdornment: filters.fechaDesde && (
        ...
        <TextField
          ...
          InputLabelProps={{ shrink: true }}
          InputProps={{ endAdornment: filters.fechaHasta && (
            <InputAdornment position="end">
        ...
        <Autocomplete
          ...
          onInputChange={(_, value, reason) => {
            setCatInput(value);
            if (reason === 'input' && value.trim().length >= 2) fetchCategories(value);
          }}
          noOptionsText={catInput.trim().length < 2 ? 'Empieza a escribir…' : 'Sin resultados'}
          ...
          onChange={(_, option) => onFiltersChange({ ...filters, categoria: option?.value ?? undefined })}
          renderInput={params => (
            <TextField
              {...params}
              label="Categoría"
