...
const columns = (map?: Record<number, string>): Column<InversionHuerta>[] => [
  { label: 'Fecha', key: 'fecha', render: i => new Date(i.fecha).toLocaleDateString('es-MX') },
  ...
  { label: 'Total',        key: 'gastos_totales',   align: 'right', render: i => dinero(Number(i.gastos_totales)) },
  { label: 'Categoría',    key: 'categoria',        render: i => map?.[i.categoria] ?? `#${i.categoria}` },
  {
    label: 'Estado', key: 'is_active', align: 'center',
    render: i => i.is_active ? (<Chip label="Activa" size="small" color="success"/>) : (<Chip label="Archivada" size="small" color="warning"/>)
  },
];
