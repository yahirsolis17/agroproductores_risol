...
export function useEmpaques(autoFetch: boolean = true) {
  ...
  }, [autoFetch, dispatch, filtersKey]); // filtersKey controla el trigger

  const refetch = useCallback(
    (override?: EmpaquesFilters) => {
  ...
  const getById = useCallback(
    (id: number) => dispatch(fetchEmpaqueById(id)),
    [dispatch]
  );

  const add = useCallback(
    (dto: EmpaqueCreateDTO) => dispatch(createEmpaque(dto)),
    [dispatch]
  );
  ...
  const archive = useCallback(
    (id: number) => dispatch(archivarEmpaque(id)),
    [dispatch]
  );

  const bulkUpsert = useCallback(
    (dto: EmpaqueBulkUpsertDTO) => dispatch(bulkUpsertEmpaques(dto)),
    [dispatch]
  );
