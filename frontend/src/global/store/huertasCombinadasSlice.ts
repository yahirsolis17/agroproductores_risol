...
import {
  huertasCombinadasService,
  Estado,
  HCFilters,
  RegistroCombinado
} from '../../modules/gestion_huerta/services/huertasCombinadasService';
...
>(
  ...
  async ({ page, estado, filters }, { rejectWithValue }) => {
    try {
      const { huertas, meta } = await huertasCombinadasService.list(page, estado, filters);
      return { huertas, meta, page };
    } catch (err: any) {
      return rejectWithValue(
        err.response?.data?.message ?? 'Error al cargar huertas combinadas'
      );
    }
  }
);
...
const hcSlice = createSlice({
  ...
  extraReducers: (b) => {
    ...
    b.addCase(fetchHuertasCombinadas.rejected,  (s, { payload, error }) => {
      s.loading = false;
      s.error   = payload ?? error.message ?? 'Error desconocido';
    });
  },
});
