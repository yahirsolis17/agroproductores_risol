...
interface CosechasState {
  ...
  page: number;
  meta: PaginationMeta;
  extra: { total_registradas?: number };
...
export const fetchCosechas = createAsyncThunk<
  { cosechas: Cosecha[]; meta: PaginationMeta; extra: { total_registradas?: number }; page: number },
  void,
...
>(
  ...
  async (_, { getState, rejectWithValue }) => {
    ...
    try {
      ...
      return { cosechas: res.data.results, meta: res.data.meta, extra: res.data.extra ?? {}, page };
    } catch (err: any) {
